...
The repository is ready for developer integration of the visual-ingestion layer. The identifier master catalogue remains authoritative and is not rewritten by the visual pipeline.

## What is implemented

- deterministic visual ingestion script;
- ingestion manifest contract and example;
- generated visual sidecar contract;
- image hashing for locally available source files;
...
### 1. Storage adapter
Connect `file_path`, `storage_key`, `url`, `thumbnail_url` and `original_image_ref` to the production object/image store. Preserve the original upload as immutable evidence.
...
### 5. Review queue
Expose `pending_review`, `verified`, `rejected` and `archived` states. AI output remains advisory until approved by the review workflow.
...
- `schema/vinmerge-generated-visual-index.schema.json`
